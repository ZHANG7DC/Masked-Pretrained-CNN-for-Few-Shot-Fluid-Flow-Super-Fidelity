...
class Transformer(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.norm(x)
class ViT(nn.Module):
    def __init__(self, *, image_size, image_channels, patch_size, dim, depth, heads, mlp_dim, channels = 2, dim_head = 32, dropout = 0., emb_dropout = 0.):
        super().__init__()
        ...
        self.sim_rearrange = Rearrange('b cf ci (h p1) (w p2) -> b (cf h w) (ci p1 p2)', p1 = patch_height, p2 = patch_width)
        self.sim_to_patch_embedding = nn.ModuleList([nn.Sequential(nn.LayerNorm(patch_dim),
                                                     nn.Linear(patch_dim, dim),
                                                     nn.LayerNorm(dim))
             for _ in range(channels)
        ])
        self.pos_embedding = nn.Parameter(torch.randn(1, num_patches_per_channel, dim))
...
class MAE(nn.Module):
    ...
    def forward(self, sim, return_value='recon_loss'):
        device = sim.device
